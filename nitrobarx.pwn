...
{
	if(HOLDING(KEY_ACTION) || HOLDING(KEY_FIRE))PlayerUsingNitro[playerid] = true;
	else if(RELEASED(KEY_ACTION) || RELEASED(KEY_FIRE))PlayerUsingNitro[playerid] = false; RemoveVehicleComponent(GetPlayerVehicleID(playerid), 1010);
	return true;
}
